...
- **Cred band** (`components/Cred.tsx`, data in `cred` in `lib/site.ts`): 45+ years (15+20+12; Cornelius corrected 25→20 for Markenfilm, CR Jul 2026), the five houses, the independence line — directly under the home hero, on ink.
...
- **Cornelius comment round (CR, Jul 2026)** — shipped across every page: engage-deck names the three entries (audit · workshop · pilot) and says "a fee agreed before we start"; audit body carries NO savings percentage (rule: no unevidenced numbers anywhere), reframed to "…mapped and scored across all six dimensions of the Method, against rates we have negotiated ourselves. Where cost leaks, you see it line by line."; "Method" in the audit card links to /approach via shared `EntryCards` (`.mlink`); Approach gets the Method-frames-every-engagement deck, "Scored red, amber or green" (was RAG-scored) and "We know it from the inside" (was "Nobody knows"); network deck promises "what they do, and when they come in"; contact Role/Need/Timing marked "· optional", a GDPR line links /privacy, "Later this year" replaces "Later in 2026"; "45-minute" hyphenated everywhere; cred lead-in is "Experience built at". Cornelius: role "Strategy, production controlling and executive production", card "Twenty years at Markenfilm", new four-paragraph bio.
- **Legal pages `/imprint` + `/privacy`** (German, `.legal`, footer "Impressum · Datenschutz") ship as **DRAFTs** — a clay draft-banner plus `[PLACEHOLDER]` fields. ⚠️ **Blocked on Justin**: street address · Geschäftsführer names · Amtsgericht + HRB · USt-IdNr. · enquiry-retention period. Fill them, then remove the banners and have a lawyer review both. NEVER remove a banner while any placeholder remains.
