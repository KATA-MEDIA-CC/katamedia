...
export function autoReplyHtml(name: string): string {
  ...
  return shell(`
        ...
        <p style="${P}">Innerhalb von 24 Stunden bekommst du eine ehrliche Einsch&auml;tzung, ob wir die Richtigen daf&uuml;r sind &mdash; und mit welchem Gr&uuml;nder du arbeiten w&uuml;rdest. Es antwortet ein Mensch, keine Standardmail.</p>
        <p style="${P}">Bis gleich,<br>Justin, Cornelius &amp; Jankel</p>
        <hr style="${RULE}">
        <p style="${MUTED}"><em>English:</em> Thank you &mdash; your enquiry has arrived. Within 24 hours you&rsquo;ll get an honest read on whether we&rsquo;re the right people to help, and which founder you&rsquo;d be working with. A person will reply, not a template.</p>`);
}
